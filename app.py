...
def main_route():
    if request.headers.get('secret_key', '') != os.environ.get('SECRET_KEY', ''):
        return utils.error_sim("InvalidSecretKey", f"The secret key is invalid or not given"), 401
    if request.method == 'PUT':
        if 'directory' not in request.headers:
            return utils.error_sim("BadRequest", "You must provide a valid format in the headers, key 'directory'",
                                   {'key': 'directory'}), 400
        directory = request.headers['directory'].replace('/', '')
        if os.path.isdir(f"uploads/{directory}"):
            return utils.error_sim("DirAlreadyExists", f"the specified directory {repr(directory)} already exists",
...
def directory_route(directory):
    ...
    if request.method == 'GET':
        if not os.path.isdir(f"uploads/{directory}"):
            return utils.error_sim("DirNotFoundError", f"the specified directory {repr(directory)} doesn't exist",
                                   {'directory': directory}), 415
        datas = []
        ...
        if not f:
            return utils.error_sim("MissingFileError", "You must provide a valid file in the body, key 'file'",
                                   {'key': 'file'}), 400
        ...
        if 'name' not in request.headers:
            return utils.error_sim("BadRequest", "You must provide a valid name in the headers, key 'name'",
                                   {'key': 'name'}), 400
        new_name = request.headers['name'].replace('/', '')
        if os.path.isdir(f"uploads/{new_name}"):
            return utils.error_sim("DirAlreadyExists", f"the specified directory {repr(new_name)} already exists",
...
def file_route(directory, file):
    ...
    if not os.path.isfile(f"uploads/{directory}/{file}"):
        return utils.error_sim("FileNotFoundError",
                               f"the specified file {repr(file)} doesn't exist in {repr(directory)}",
    ...
    if request.method == 'GET':
        ...
        if not request.json:
            return utils.error_sim("BadRequest", "You must provide a json in the body"), 400
